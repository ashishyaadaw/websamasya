...
if (isset($_GET['update'])) {
 ...
 echo ' 
      <form action="update.php" method="POST"  enctype="multipart/form-data" class="crud-form ft-andika">
        ...
        <label for="image">slide ID:</label>
        <input type="text" id="slideId" name="key" value="'.$updateIndex.'" required />
...
echo '<div  class="row d-flex no-input-box ft-quicksand"> <form> <div
            id="delete_slide_key_'.$index.'"
          ...
          echo ' <div class="card-header">'.$index.'</div>
           ...
           echo '<h5 class="card-title">'.$data[$index]['title'].'</h5>';
           echo '<p class="card-text">
               '.$data[$index]['quote'].'
              </p>
              <br/>
              <img id="preview" src="'.$data[$index]['image'].'" height="100px" />
              ';
  ...
  echo ' <br/><small><strong>Image SRC :</strong> '.$data[$index]['image'].'</small><br />';
              echo '<br><button
                type="button"
                onclick='."'".'setBlock("edit_slide_key_'.$index.'"), setNone("delete_slide_key_'.$index.'")'."'".'
    ...
    echo '		<form action="update.php" method="post"  enctype="multipart/form-data">
					...
					>
						...
						<div class="card-body">
							<h5 class="card-title">
								<input type="text" id="sildeTitle" name="slideTitle" value="'.$data[$index]['title'].'" />
							</h5>
							<p class="card-text">
				<input type="text" id="slideParagraph" name="slideParagraph" value="'.$data[$index]['quote'].'" />
							</p>
							<br />

							<div class="row d-flex">

								<input type="file" name="file" id="slideFile" 
									accept="image/*" />
								// <input style="color:blue;background:#f5f5f5;" type="text" name="slideUrl"  value="'.$data[$index]['href'].'" />
