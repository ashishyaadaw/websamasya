...
if (isset($_POST['create'])) {
    ...
    && $imageFileType != "gif" && $imageFileType != "pdf" ) {
        if($imageFileType == null) {
            ...
            <h1 class='text-center'>testmonial created with URL</h1> 
</div>" ;
        $uploadOk = 2;
        } else {
    ...
    // if everything is ok, try to upload file
        ...
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFile)) {
            echo "
            <div class='alert alert-success' role='alert'>
           <h1 class='text-center'> Successfully added testmonial date created with file ". htmlspecialchars( basename( $_FILES["file"]["name"])). "</h1>
</div>"
          ...
          ;
            ...
            <span>testmonial created with URL<span> 
</div> " ;
            }else{
...
if (isset($_GET['update'])) {
 ...
 echo ' 
      <form action="update.php" method="POST"  enctype="multipart/form-data" class="crud-form ft-andika">
        ...
        <label for="image">testmonial ID:</label>
        <input type="text" id="testmonialId" name="key" value="'.$updateIndex.'" required />
...
echo '<div class="row row-reverse d-flex no-input-box ft-quicksand"> <form> <div
            id="delete_testmonial_key_'.$index.'"
            class="card text-bg-light mb-3 m-3"
            style="max-width: 30rem"
          >';

          echo ' <div class="card-header">'.$data[$index]['about'].'</div>
           ';

           echo '<div class="card-body">';
           echo '<h5 class="card-title">'.$data[$index]['name'].'</h5>';
           echo '<p class="card-text">
              ...
              <br/>';
				...
				<input type="checkbox" id="visibility" name="isNew" value="true" '.$isChecked.' /></label>';
  echo ' <small>Link : <a href="'.$data[$index]['social_links'].'">'.$data[$index]['social_links'].'</a></small
              ><br />';
              echo '<br><button
                type="button"
                onclick='."'".'setBlock("edit_testmonial_key_'.$index.'"), setNone("delete_testmonial_key_'.$index.'")'."'".'
    ...
    echo '		<form action="update.php" method="post"  enctype="multipart/form-data">
					...
					>
						<div class="card-header">
							...
							<input type="text" id="testimonialAbout" name="about" value="'.$data[$index]['about'].'" />
						</div>
						<div class="card-body">
							<h5 class="card-title">
								<input type="text" id="testimonialName" name="name" value="'.$data[$index]['name'].'" />
							</h5>
							<p class="card-text">
								<input
									type="text"
									id="testimonialQuote"
                                    name="quote"
									value="'.$data[$index]['quote'].'"
									placeholder="'.$data[$index]['quote'].'"
								></input>
							</p>
							<br />
							...
							<div class="row d-flex">
								...
								<input type="file" name="file" id="testimonialFile" accept="image/*"/>
								<input style="color:blue;background:#f5f5f5;" type="text" name="socialLink"  value="'.$data[$index]['social_links'].'" />

							</div>
                                           <hr />
                  <div class="row d-flex ">
                            <label class="col" for="">Visibility:</label>
							<input class="col" type="radio" name="visibility" value="true" '.$isChecked.'/>
                            <label class="col" for="visibility">YES</label>
							<input class="col" type="radio" name="visibility" value="false" '.$NoChecked.'/>
							<label class="col" for="visibility">NO</label>
